...
public class HuffmanTreeNode {
	...
	public static Comparator<HuffmanTreeNode> compareWeightOrd = new Comparator<HuffmanTreeNode>() {
		...
		public int compare(HuffmanTreeNode ht1, HuffmanTreeNode ht2) {
			...
			else if(ht2.getWeight() > ht1.getWeight()) return -1;
			else {
				if (ht1.getOrdValue() > ht2.getOrdValue()) return 1;
				else if(ht2.getOrdValue() > ht1.getOrdValue()) return -1;
				else {
					if(ht1.getId() > ht2.getId()) return 1;
					else if(ht2.getId() > ht1.getId()) return -1;
				}
			}
			return 0;
		}
