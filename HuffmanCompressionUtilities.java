...
public class HuffmanCompressionUtilities {
	...
	public HuffmanCompressionUtilities() {
		...
		for (int i = 0; i < NUM_ASCII; i++) weights[i] = 0;
	...
	void buildHuffmanTree(boolean minimize) {
		HuffmanTreeNode left = null, right = null, node;
		this.root = null;
		encodeMap = new String[NUM_ASCII];
		initializeHuffmanQueue(minimize);
		while (!queue.isEmpty()) {
			left = queue.poll();
			int curWeight = left.getWeight();
			if (queue.isEmpty()) {
				root = left;
				return;
			}
			right = queue.poll();
			curWeight += right.getWeight();
			node = new HuffmanTreeNode(curWeight, left, right);
			queue.add(node);
		}
	}
	...
	void createHuffmanCodes(HuffmanTreeNode node, String code, int level) {
		//TODO: write this method
		if(node == null) return;
		if(node.getLeft() != null) createHuffmanCodes(node.getLeft(), (code + "0"), level+1);
		if(node.getRight() != null) createHuffmanCodes(node.getRight(), (code + "1"), level+1);
		if(node.isLeaf()) encodeMap[node.getOrdValue()] = code;
	}
	...
	private int traverseTree(HuffmanTreeNode root, String binStr, int index) {
		//TODO - write this method
		if (root.isLeaf() == true) return root.getOrdValue();
		else if (index >= binStr.length()) return -1;
		else if (binStr.charAt(index) == '0' ) return traverseTree(root.getLeft(), binStr, index + 1);
		else if (binStr.charAt(index) == '1') return traverseTree(root.getRight(), binStr, index + 1);
		return -1;
	}
	...
	int decodeString(String binStr) {
		//TODO - write this method
		if(binStr.isEmpty())return -1;
		else return traverseTree(this.getTreeRoot(), binStr, 0);
	}
